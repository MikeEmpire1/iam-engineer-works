...
└── Management OU (delegated admin)
    └── Management (org root, billing, IAM Identity Centre)
```
...
|---|---|---|---|---|
| **Management** | `1111-2222-3333` (`inno-mgmt`) | Management | Organization root, billing consolidation, IAM Identity Centre, CloudTrail org trail | AWS Organizations, IAM Identity Centre, Consolidated Billing, S3 (access logs) |
| **Security** | `4444-5555-6666` (`inno-security`) | Security | Centralised security monitoring and auditing | Amazon GuardDuty, Security Hub, AWS Config, CloudTrail, KMS, Access Analyser, Detective |
...
### Authentication
- **IAM Identity Centre** is the single pane of glass for AWS access
- Entra ID users (HR, Finance, Legal, Exec) are synced via SCIM to IAM Identity Centre
- Engineering & IT users are managed directly in IAM Identity Centre
- Sales & Marketing (guest accounts) have no AWS access by default

### Authorisation
- Access is group-based via **Permission Sets** in IAM Identity Centre
...
Denies all AWS services except those required for organisation management, identity, billing, and audit logging. Excludes the break-glass role so emergency access remains functional.
...
In the event of an IAM Identity Centre outage or loss of administrative access:
