...
InnoGrid's SOC 2 Type II and ISO 27001 certifications require **quarterly access reviews**. Every manager must recertify their direct reports' access to AWS accounts at least once per quarter. Access that is not recertified within the review window is automatically revoked.

The review covers:

- **IAM Identity Center users** (Engineering & IT) — group memberships and permission set assignments
- **Entra ID synced users** (Corporate) — group memberships in IAM Identity Center
...
| Carlos Mendez | Helpdesk (Emily Zhao) — Kevin Nguyen terminated | 1 |
...
| Emily Zhao | Carlos Mendez | **Approve** | Active helpdesk tech |
...
1. **Campaign setup** — Create a Q3 2026 access review campaign covering all IAM Identity Center users with AWS access
